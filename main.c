...
void SIM_Send(char* cmd){
	int i = strlen(cmd);
	HAL_UART_Transmit(&huart1,(uint8_t *) cmd, i, 1000);
}
/* SIM Send AT Command*/
void SIM_sendCommand(char* cmd) {
  SIM_Send(cmd);
...
char* SIM_replyCommand(int delay){
		HAL_UART_Receive(&huart1, (uint8_t *)dataGPRS.reply, 100, 1000);
		printf("%s\n",dataGPRS.reply);
		HAL_Delay(delay);
...
/* Sim function*/
/* _____________________________SIM_SMS________________________ */
